...
public:
	...
	virtual void pollSpecificEvents() {
		if(static_cast<WinAPIWindow*>(m_windowImpl)->m_event.message == WM_KEYDOWN) {
			m_keyData.keyEnum = translateFromVirtualKey(static_cast<WinAPIWindow*>(m_windowImpl)->m_event.wParam);
		}
		else if(static_cast<WinAPIWindow*>(m_windowImpl)->m_event.message == WM_CHAR) {
			m_keyData.keyChar = static_cast<WinAPIWindow*>(m_windowImpl)->m_event.wParam;
		}
	}
